...
class SwitchNode(ASTNode):
    def __init__(self, line_num, expr_str, cases):
        self.line_num = line_num
        self.expr_str = expr_str.strip()
        self.cases = cases  # list of dict: {'type': 'case'/'default', 'val_str': str or None, 'body': list of ASTNode, 'line_num': int}
...
def extract_block(lines, start_idx):
    """
    Finds matching curly brace block starting from start_idx in lines.
    Returns the block lines (excluding outer curly braces), the end line index, and the matching brace char index.
    """
    brace_count = 0
    found_open = False
    start_search_idx = -1
    ...
    # 2. Find the matching closing brace '}' starting from the character right after the first '{'
    brace_count = 1
    end_idx = start_search_idx
    matching_brace_char_idx = -1
    
    # We scan the rest of the line containing the first '{'
...
def parse_switch_cases(lines, start_line_num):
    cases = []
    idx = 0
    current_case = None
    ...
    while idx < len(lines):
        ...
        case_match = re.match(r'^case\s+(.*?)\s*:', line_stripped)
        default_match = re.match(r'^default\s*:', line_stripped)
        
        if case_match:
            val_str = case_match.group(1)
            current_case = {'type': 'case', 'val_str': val_str, 'body': [], 'line_num': line_num}
            cases.append(current_case)
            idx += 1
            colon_idx = line.find(':')
            rem = line[colon_idx+1:].strip()
            if rem:
                current_case['body'].extend(parse_block([rem], line_num))
            continue
            
        elif default_match:
            current_case = {'type': 'default', 'val_str': None, 'body': [], 'line_num': line_num}
            cases.append(current_case)
            idx += 1
            colon_idx = line.find(':')
            rem = line[colon_idx+1:].strip()
        ...
        if current_case is not None:
            # Parse one statement/block structure
            # 1. FOR
            ...
            if for_match:
                ...
                if len(header_parts) == 3:
                    ...
                    current_case['body'].append(ForLoopNode(line_num, init_str, cond_str, step_str, body_nodes))
                    idx = end_idx + 1
                    continue
            ...
            if while_match:
                ...
                body_lines, end_idx, _ = extract_block(lines, idx)
                body_nodes = parse_block(body_lines, line_num)
                current_case['body'].append(WhileLoopNode(line_num, cond_str, body_nodes))
                idx = end_idx + 1
                continue
            ...
            if do_match:
                ...
                current_case['body'].append(DoWhileLoopNode(line_num, cond_str, body_nodes))
                idx = end_idx + 1
                continue
            ...
            if if_match:
                ...
                current_case['body'].append(if_node)
                idx = end_idx + 1
                continue
                
            # 5. Standard Statement
            current_case['body'].append(StatementNode(line_num, line_stripped))
            idx += 1
        else:
            idx += 1
...
def parse_block(lines, start_line_num):
    """
    Parses a flat list of lines into structured ASTNode list.
    """
    nodes = []
    idx = 0
    ...
    while idx < len(lines):
        ...
        # Ignore class headers, packages, imports, and main method declarations
        ...
        # Handle matching closing braces of outer main blocks
        if line_stripped == '}':
            idx += 1
            continue
        ...
        # 2.7. SWITCH selection detection
        switch_match = re.match(r'^\s*switch\s*\((.*?)\)\s*\{?', line_stripped)
        if switch_match:
            expr_str = extract_parentheses_content(line_stripped, "switch")
            body_lines, end_idx, _ = extract_block(lines, idx)
            cases = parse_switch_cases(body_lines, line_num)
            nodes.append(SwitchNode(line_num, expr_str, cases))
            idx = end_idx + 1
            continue
...
class JavaTracer:
    def __init__(self, code):
        self.raw_code = code
        self.variables = {}  # name -> { 'value': any, 'type': str }
        self.console_output = ""
        self.steps = []
        self.nodes = []
    ...
    def eval_expr(self, expr, line_num):
        ...
        # Sort variables by length descending to prevent replacing substrings of variable names
        ...
        # Replace casts with python helper keywords
    ...
    def log_step(self, line_num, narration, changed_var=None):
        # Create a deep copy of current variables state
        vars_copy = {}
        ...
        self.steps.append({
            'lineNumber': line_num + 1,  # 1-indexed for the visualizer
            'variables': vars_copy,
    ...
    def run_statement(self, node: StatementNode):
        ...
        if arr_assign_match:
            ...
            self.variables[arr_name]['value'] = list(curr_list)
            self.log_step(node.line_num, f"Updated {arr_name}[{idx}] to {new_val}.", changed_var=arr_name)
            return

        # 4. Standard Variable declaration: e.g. "int sum = 0"
        ...
        # 5. Standard Assignment updates: e.g. "sum += i" or "x = y * 2"
        ...
        # 6. Post/Pre Increment/Decrement: e.g. "i++"
        ...
        # 7. Print statements: e.g. "System.out.println(sum)"
        ...
        # If it reached here, the line could not be parsed as a supported Java statement
    ...
    def run_nodes(self, nodes):
        for node in nodes:
            if isinstance(node, StatementNode):
                ...
                # Create a temporary StatementNode for init code
                ...
                while True:
                    loop_counter += 1
                    if loop_counter > 500:  # Prevent infinite loops in sandbox!
                ...
                switch_val = self.eval_expr(node.expr_str, node.line_num)
                self.log_step(node.line_num, f"Evaluating switch expression ({node.expr_str}): {switch_val}")
                
                # 2. Match cases
                matched_idx = -1
                for idx_case, case in enumerate(node.cases):
                    if case['type'] == 'case':
                        case_val = self.eval_expr(case['val_str'], case['line_num'])
                        if case_val == switch_val:
                            matched_idx = idx_case
                            break
                ...
                # 3. Execute matched cases (with fall-through!)
                if matched_idx != -1:
                    try:
                        for idx_case in range(matched_idx, len(node.cases)):
                            self.run_nodes(node.cases[idx_case]['body'])
                    except BreakException:
...
if __name__ == "__main__":
    ...
    """
    tracer = JavaTracer(test_code)
    res = tracer.trace()
    if 'error' in res:
        print("Tracer Error:", res['error'])
    else:
        print("Steps generated:")
        for idx, s in enumerate(res['steps']):
            print(f"Step {idx+1} [Line {s['lineNumber']}]: {s['narration']}")
            print("  Vars:", s['variables'])
            print("  Console:", repr(s['output']))
